    ...
    <div>
      ...
      <modal :name="modal_key" :adaptive="true" :scrollable="true" height="auto" :min-height="400" style="top: 3rem;">
        <div class="p-6 text-gray-700">
          <h1 class="font-bold text-lg">Asignar checklist</h1>
          <br>
  
          <form @submit.prevent="assignChecklist">
            <div class="lg:px-3 lg:mb-0 mb-3 mt-3">
              <label class="form-label">Listado</label>
              <select class="form-input" required v-model="form.checklist_id">
                <option v-for="(item, index) in checklists" :key="index" :value="item.id">
                  {{ item.name }}
                </option>
              </select>
            </div>
