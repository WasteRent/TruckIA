...
@section('content')
	...
	@include('fleet.containers.edit_tabs', ['active_incidents' => true])

	
	@component('components.card', ['compressed' => true])
    @slot('title', __('Añadir incidencia'))
    @include('fleet.containers.incidents.create')
@endcomponent
...
{!! 
  Form::model(request()->all(), [
    'route' => ['fleet.containers.incidents.index', $container], 
    'method' => 'GET',
          ...
          <tbody>
              ...
              <tr>
                ...
                <td class="w-full">
                  ...
                  <form class="incidence_form hidden" method="POST" action="{{ route('fleet.containers.incidents.update', [$container, $incidence->id]) }}">
                    @csrf
                    @method('PUT')
                ...
                <td>
                  @if($incidence->closed_at)
                    <x-form-button method="PUT" :action="route('fleet.containers.incidents.update', [$container, $incidence->id])" class="btn-outline-gray">
                        <input type="hidden" name="reopen" value="1">
                        {{ __('Reabrir') }}
                    </x-form-button>
                  @else
                    <x-form-button method="PUT" :action="route('fleet.containers.incidents.update', [$incidence->container, $incidence->id])" class="text-xs flex items-center text-red-700">
                    ...
                    <a class="text-xs flex items-center text-blue-700 mt-3 w-24" href="{{ route('fleet.fast-orders.create', ['container_id' => $incidence->container->id, 'incident_id' => $incidence->id]) }}">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <form class="mt-3" method="POST" onsubmit="return confirmDelete()" action="{{ route('fleet.containers.incidents.destroy', [$container, $incidence]) }}">
                    @csrf
                    @method('DELETE')
