		...
		@if(!$repair_order->isFinished())
			@slot('corner')
				<div class="flex gap-2">
					...
					@endif
					@if(in_array(Auth::user()->job, ['fleet_manager', 'garage_boss', 'zone_administrator', 'zone_administrator']))
			...
			<div class="sm:w-1/2">
				@component('components.table')
					@slot('items', [
						...
						__('Garantía asociada') => $repair_order->related_guarantee_id ? "#{$repair_order->related_guarantee_id} - {$repair_order->relatedGuarantee->user->name}" : null,
		...
		{!! Form::model($repair_order, [
			...
			<div class="flex flex-wrap -mx-3 mb-6">
				<div class="w-full md:w-2/12 px-3 mb-6 md:mb-0">
				  ...
				  {!! Form::select('type', ['preventive' => __('Preventivo'),'corrective' => __('Correctivo'),'pre-itv' => __('Pre-ITV'), 'weekly' => __('Semanal'), 'tires' => 'Neumáticos', 'bad_use' => 'Malos usos', 'support' => __('Asistencia')], request()->query('type'), ['class' => 'form-select']) !!}
				</div>
				<div class="w-full md:w-2/12 px-3 mb-6 md:mb-0">
				<label class="form-label" >
