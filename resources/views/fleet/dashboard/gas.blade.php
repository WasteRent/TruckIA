...
@section('title', __('Revisión de gas'))

@section('content')

	@include('fleet.dashboard.tabs2', ['gas' => true])

	@component('components.search-card')
		{!! 
			Form::model(count(request()->all()) > 0 ? request()->all() : session('filters'), [
				'route' => 'fleet.dashboard.gas', 
				'method' => 'GET',
	...
	<div class=""> 
		...
		@foreach($comming->groupBy(function($i) {
			return Carbon\Carbon::parse($i->gas_revision_date)->format('M-Y');
		}) as $month => $vehicles)
			...
			<div class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
				@foreach($vehicles as $vehicle)
					@include('fleet.dashboard.gas_card')
				@endforeach
			</div>
		@endforeach
