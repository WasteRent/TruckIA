...
@section('content')
		...
		<div class="flex">
			@if($model->technicalHandbook)
				<div class="w-1/2 flex items-center">
					...
					<form method="POST" onsubmit="return confirmDelete()" action="{{ route('fleet.handbooks.technical.destroy', $model) }}">
						@csrf
						@method('DELETE')
			...
			@if($model->usageHandbook)
				<div class="w-1/2 flex items-center">
					...
					<form method="POST" onsubmit="return confirmDelete()" action="{{ route('fleet.handbooks.usage.destroy', $model) }}">
						@csrf
						@method('DELETE')
						<button><i class="icon fas fa-trash-alt"></i></button>
					</form>
	...
	@if(!$model->technicalHandbook)
		@component('components.card')
			...
			{!! Form::open([
			  'route' => ['fleet.handbooks.technical.store', $model],
			  'files' => true,
			  'method' => 'POST',
			  'class' => 'w-full'
			]) !!}  
	...
	@if(!$model->usageHandbook)
		@component('components.card')
			...
			{!! Form::open([
			  'route' => ['fleet.handbooks.usage.store', $model],
			  'files' => true,
			  'method' => 'POST',
			  'class' => 'w-full'
			]) !!}  
