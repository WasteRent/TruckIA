@php
	$enlaces[] =
		[
			...
			'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'garage_boss', 'mechanic', 'garage', 'contract_manager', 'zone_administrator'])
		];
	$enlaces[] =
		[
			...
			'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'mechanic', 'zone_administrator'])
		];
	$enlaces[] =
		[
			'name' => __('Preventivos'),  
	...
	$enlaces[] =
		[
			...
			'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'capataz', 'garage_boss', 'garage', 'mechanic', 'contract_manager', 'zone_administrator'])
		];
	$enlaces[] =
	[
		'name' => __('Incidencias'),  
	...
	[
		...
		'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'vehicle_washing', 'garage_boss', 'mechanic', 'zone_administrator']) || in_array(auth()->user()->username, ['victor1270', 'manuel1284'])
	];
	...
	$enlaces[] =
		[
			...
			'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'garage_boss', 'garage', 'contract_manager', 'zone_administrator'])
		];
	$enlaces[] =
			[
				...
				'active' => request()->is('fleet/containers*') || request()->is('fleet/container-checklists*'),
				'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'zone_administrator', 'mechanic', 'garage_boss', 'container_manager'])
			];
	$enlaces[] =
		[
			'name' => __('Recambios'),
	...
	if(in_array(Auth::user()->id, [3,920, 929,872,637, 928,955,904,1034,1033,1413,2169,1035,2222,892,904,664,2432]) || in_array(Auth::user()->job, ['contract_manager', 'zone_administrator'])) {
		$enlaces[] =
				[
		...
		$enlaces[] =
				[
					...
					'disponible' => in_array(auth()->user()->job, ['fleet_manager', 'contract_manager', 'zone_administrator'])
				];
		$enlaces[] =
				[
