...
class FleetVehicleImport implements ToModel, WithHeadingRow, WithValidation, SkipsEmptyRows
{
    ...
    protected function createVehicle(array $row)
    {
        return Vehicle::updateOrCreate(
            ...
            'warranty_date' => $row['fecha_garantia'] != '' ? $row['fecha_garantia'] : null,
            'itv_date' => $row['fecha_proxima_itv'] != '' ? $row['fecha_proxima_itv'] : null,
            'tachograph_date' => $row['fecha_proximo_tacografo'] != '' ? $row['fecha_proximo_tacografo'] : null,
            'kms' => (int) $row['kms'],
