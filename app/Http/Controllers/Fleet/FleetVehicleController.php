...
class FleetVehicleController extends Controller
{
    public function index(Request $request)
    {
        ...
        $years = Vehicle::all()->pluck('manufacturing_date')->map(fn($date) => \Carbon\Carbon::parse($date)->format('Y'))->unique()->sort()->values();
        return view('fleet.vehicles.index', [
            ...
            'customers' => Customer::where('fleet_id', Auth::user()->fleet->id)->whereHas('vehicles', function ($q) {
                $q->allowForUser();
            })->orderBy('name')->get(),
